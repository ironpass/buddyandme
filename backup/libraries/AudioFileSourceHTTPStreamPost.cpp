...
retry:
  if (!http.connected()) {
    Serial.println("Stream disconnected, retrying connection...");
    http.end();
    if (!http.begin(*client, saveURL)) {
      Serial.println("ERROR: Can't reopen HTTP request");
      return 0;
    }
    int code = http.POST((uint8_t*)this->postData, strlen(this->postData));
    if (code != HTTP_CODE_OK) {
      http.end();
      Serial.println("ERROR: Can't open HTTP request on retry");
      return 0;
    }
  }
  ...
  if (!nonBlock) {
    ...
    while ((retries > 0) && (stream->available() == 0)) {
      Serial.println("Waiting for data in non-blocking mode...");
